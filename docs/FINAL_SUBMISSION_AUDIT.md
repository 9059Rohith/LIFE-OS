...
| Hosted credential rotation | COMPLETE / LIVE-VERIFIED | The current Railway deployment accepts the rotated credential; the historical credential returns 401 | MEDIUM | Keep the replacement credential out of source, issues, and demo descriptions. |
| Hosted/source parity | COMPLETE / VERIFIED UPLOAD | The final checkout was uploaded with `railway up` and passed health, readiness, homepage, and authenticated login/session checks | MEDIUM | Recheck after future deployments; do not infer provider mutation coverage from an auth smoke test. |
