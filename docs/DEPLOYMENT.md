...
The current checkout is deployed at `https://lifeos-live-production.up.railway.app` as the `lifeos-live` service in EU West. The verified deployment is `5378760c-07f4-4d71-b337-4d5b4bfb746b`, uploaded from checkout commit `81ec67f`. It uses one replica and its own volume mounted at `/app/data`; its database URL is `sqlite:////app/data/lifeos.db`. The service has `LIFEOS_MODE=live`, `LIFEOS_ENVIRONMENT=production`, a password, encryption key, HTTPS allowed origin, Google client settings, Discord bot settings and server-side OpenAI settings. The exact credentials are stored in Railway variables, not in this repository. Post-deploy health, readiness, homepage, rotated-credential login, and authenticated session checks passed on 20 September 2026. The historical credential returned 401 after rotation. See [release status](RELEASE_STATUS.md) for provider and deployment boundaries.

The hosted owner now holds the existing owner's Google grant, re-encrypted under the hosted key after a volume backup. Hosted Gmail, Calendar and Drive passed live read checks on 18 September 2026. The exact callback `https://lifeos-live-production.up.railway.app/api/integrations/google/callback` was registered with Google and a fresh authorization request no longer failed with `redirect_uri_mismatch`. Calendar write access passed a temporary private-event create/read/delete probe. Do not copy the local `.env` or database into the public service. WhatsApp can use the signed-in desktop view through the owner-scoped bridge; it does not require Chromium in the image. The older `lifeos-public` service remains an isolated demo.
...
For the signed-in Windows desktop bridge, set the exact allowed chat display name in `LIFEOS_WHATSAPP_CONTACT` and enable `LIFEOS_WHATSAPP_BRIDGE_ENABLED=true` on the live backend. Keep one backend replica and one desktop app connected. The desktop app uses its persistent WhatsApp Web view for check, read, approved send and read-back jobs. Each short-lived job requires the owner session and CSRF token; a timeout during a send remains uncertain and cannot be replayed automatically. No WhatsApp website credentials are copied to Railway. Sign in to WhatsApp Web manually inside the desktop app before checking Integrations. The live send path still needs an approved message and independent delivery acceptance before release claims.

The alternative Playwright worker below is for a dedicated local profile; do not enable both paths for the same account.
...
Use your database platform's encrypted automated backups and point-in-time recovery. The current Railway live service uses SQLite, not the optional Compose PostgreSQL service. Its [SQLite recovery procedure](RECOVERY.md) includes online snapshots, authenticated encryption, archive verification and isolated restore; an external destination and schedule still require operator configuration. Store the token encryption key in a separate recoverable secret store. Validate restoration to an isolated database before launch. Backups contain personal event data and must follow retention rules. Named Docker volumes are persistence, not backups.
